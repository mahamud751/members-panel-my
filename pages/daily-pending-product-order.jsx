...
const DailyPendingProductOrder = (props) => {
  ...
  const getDailyPendingProductOrder = props.data;
  console.log(getDailyPendingProductOrder);
  const [ProductStockQty, updateDailyPendingOrder] = useRecoilState(storePendingListOfProduct);

  useEffect(() => {
    updateDailyPendingOrder(getDailyPendingProductOrder);
  }, [updateDailyPendingOrder]);
  ...
  //     });
  // };
  const dailyPendingOrders = useRecoilValue(storePendingListOfProduct);
  console.log(dailyPendingOrders);
  return (
    <div>
      <div className="row">
            ...
            <div className="card-body">
              <div className="datatable-wrapper table-responsive">
                <table id="datatable" className="display compact table table-striped table-bordered">
                  ...
                  <tbody>
                    {dailyPendingOrders.map((item, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        <td>{item.userId}</td>
                        <td>
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/userPanel/v1/GetDailyPendingOrderList/" + decodedToken.userId);
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
