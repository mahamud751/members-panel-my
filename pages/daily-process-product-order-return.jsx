...
const DailyProcessProductOrderReturn = (props) => {
  useScript("/assets/js/app.js");

  const getDailyProcessProductOrderReturn = props.data;
  console.log(getDailyProcessProductOrderReturn);
  const [ProductStockQty, updateProcessProductOrderReturn] = useRecoilState(storeReturnProcessListOfProduct);

  useEffect(() => {
    updateProcessProductOrderReturn(getDailyProcessProductOrderReturn);
  }, [updateProcessProductOrderReturn]);
  ...
  //     });
  // };
  const processProductOrderReturn = useRecoilValue(storeReturnProcessListOfProduct);
  console.log(processProductOrderReturn);
  return (
    <div>
      <div className="row">
              ...
              <nav>
                <ol className="breadcrumb p-0 m-b-0">
                  ...
                  <li className="breadcrumb-item active text-primary" aria-current="page">
                    List of daily process product order return
                  </li>
                </ol>
              </nav>
            </div>
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/userPanel/v1/GetDailyReturnProcessOrderList/" + decodedToken.userId);
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
...
export default DailyProcessProductOrderReturn;
