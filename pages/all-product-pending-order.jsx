...
import { storeTotalPendingListOfProduct } from "../atom/listOfPendingOrder";
...
const AllPendingProductOrder = (props) => {
  ...
  const [ProductStockQty, updatePendingAllProductOrder] = useRecoilState(storeTotalPendingListOfProduct);

  useEffect(() => {
    updatePendingAllProductOrder(getPendingAllProductOrder);
  }, [updatePendingAllProductOrder]);
  ...
  //     });
  // };
  const pendingAllProductOrders = useRecoilValue(storeTotalPendingListOfProduct);
  console.log(pendingAllProductOrders);
  return (
    <div>
      <div className="row">
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/userPanel/v1/GetTotalPendingOrderList/" + decodedToken.userId);
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
