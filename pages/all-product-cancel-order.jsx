...
import { storeTotalCancelListOfProduct } from "../atom/listOfMembersStore";
...
const AllCancelProductOrder = (props) => {
  ...
  const [ProductStockQty, updateCancelAllProductOrder] = useRecoilState(storeTotalCancelListOfProduct);
  ...
  //     });
  // };
  const cancelAllProductOrders = useRecoilValue(storeTotalCancelListOfProduct);
            ...
            <div className="card-body">
              <div className="datatable-wrapper table-responsive">
                <table id="datatable" className="display compact table table-striped table-bordered">
                  ...
                  <tbody>
                    {cancelAllProductOrders.map((item, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        <td>{item.userId}</td>
                        <td>
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/userPanel/v1/GetTotalCancelOrderList/" + decodedToken.userId);
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
